...
int_2179_check <- int_2179[,c("first_author","year","e_s1_o_s2", "e_unit", "o_unit","replication", "mrbeta","mrse","mrlci","mruci","mrp")]
...
same_autoimmune$pval_threshold[which(same_autoimmune$ID == 776143774 & same_autoimmune$exposure_formatted == "Allisonella (Genus)" & same_autoimmune$mrbeta >= 0.340)] <- "5x10-08"
same_autoimmune$pval_threshold[which(same_autoimmune$ID == 776143774 & same_autoimmune$exposure_formatted == "Bifidobacterium (Genus)" & same_autoimmune$mrbeta >= -0.30)] <- "5x10-08"
...
same_autoimmune_new <- same_autoimmune_new[-(which(same_autoimmune_new$main_method =="Inverse variance weighted (both fixed and random effects)")),]
same_autoimmune_new_check <- same_autoimmune_new[,c("ID","exposure_formatted","e_s1_o_s2","e_unit","o_unit","outcome_study_case","outcome_study_control","outcome_og_gwas_case","outcome_og_gwas_control","outcome_n_diff","analysis_notes","other_gwasinfo","pval_threshold","main_method","replication","mrbeta","mrse","mrlci","mruci","mrp")]
...
for(i in outcome_list){
  for(j in exposure_list){
    tryCatch({
      ...
      forest(meta_res, studlab = T, comb.fixed = F, comb.random = T, common = FALSE, digits = 3, leftcols = c("study1","studlab"), leftlabs = c("Exposure study","Outcome study"))
      grid.text(label, .5, 0.8, gp=gpar(cex=1.4))
      dev.off()
      rm(label, rep, meta_res)
    }, error=function(e){cat("Error :",conditionMessage(e), "\n")})
