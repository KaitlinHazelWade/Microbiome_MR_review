...
p1 <- ggplot(data_summary, aes(x = reorder(outcome_group, -count), y = count, fill = outcome_group)) +
...
p2 <- ggplot(data_summary, aes(x = reorder(taxa_group, -count), y = count, fill = taxa_group)) +
  geom_bar(stat = "identity", width = 0.7) +
...
htmlwidgets::saveWidget(sankey, "sankey_plot.html", selfcontained = T)
webshot::webshot("sankey_plot.html", "sankey_plot.png", vwidth = 1000, vheight = 800, delay = 0.5)  
sankey_grob <- grid::rasterGrob(png::readPNG("sankey_plot.png"), interpolate = TRUE)
sankey_plot <- plot_grid(
  ggdraw() + draw_label("Sankey Plot of exposures and outcomes", fontface = "bold", size = 14),  
...
plot_grid(
  plot_grid(p2, p1, ncol = 1, rel_heights = c(1, 1), labels = c("(A)", "(B)")),  
  sankey_plot,  
