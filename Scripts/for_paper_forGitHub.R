...
#### Quality assessment
quality <- read.xlsx("./Quality Assessment/QA_results_ForPublication_040425.xlsx",sheet = "RoB FINAL", colNames = TRUE)
quality <- quality[-c(67,68,69),] 
colnames(quality)
colnames(quality) <- c("ID","Author","Year","Title","DOI","IV.selection.(1)","IV.selection.(2)","Weak.instrument.bias","Genetic.confounding","Other.Confounding./.Collider.bias","Additional.direct.effects.between.IV.and.outcome.(exclusion.restriction.assumption)","Bias.due.to.selection.of.participants","Reverse.causation","Harmonization./.preparation","Overestimation.via.sample.overlap.or.overfitting","Main.analyses","Sensitivity.analyses","Replication","Data","Exposure.information","SNP.information","Outcome.information","Reporting","Data.availability.and.reproducibility","Count.if.any.high.risk.of.bias")
...
# Those that were over 25 are Replication, Reporting, Sensitivity analyses, IV.selection (1), data availability and reproducibility, bias due to selection of participants, then iv selection (2)
colnames(quality)
low_quality <- c("Replication","Reporting","Sensitivity.analyses","IV.selection.(1)","Data.availability.and.reproducibility","Bias.due.to.selection.of.participants","IV.selection.(2)")
...
summary(all_moderate_quality[,1]) ## maximum is 61 so let's see which are above 30
for(i in colnames(quality)){
...
# Ones that are over 12 were weak instrument bias, data, IV selection (1), exclusion restriction and reverse causation
colnames(quality)
...
paste0(quality$Author[which(quality$`IV.selection.(1)` ==3)]," ", quality$Title[which(quality$`IV.selection.(1)` ==3)])
...
## FIGURES ##
#############
## Lets process our exposure and outcome data so we can plot them
data_clean <- data %>%
  select(outcome_group, exposure_formatted) %>%
  # First edit the outcomes so that NAs are replaced with unknown Outcome
  mutate(outcome_group = ifelse(is.na(outcome_group), "Unknown Outcome", outcome_group),
         # Now we want to make a new var for the taxa group so we can process this 
         taxa_group = NA_character_) %>%
  mutate(
    taxa_group = ifelse(grepl("\\(Genus\\)$", exposure_formatted), "Genus", taxa_group),
...
## Visualisation of outcome types (original Figure 3 but now combined with Figure 4 into one Sankey plot)
data_summary <- data.frame(table(data_clean$outcome_group, useNA = "ifany"))
...
p1a <- ggplot(data_summary, aes(x = reorder(outcome_group, -count), y = count, fill = outcome_group)) +
...
p1b <- ggplot(data_summary, aes(x = reorder(outcome_group, -count), y = log(count), fill = outcome_group)) +
  geom_bar(stat = "identity", width = 0.7) +
  labs(x = "Outcome Group", y = "Log(Total Count)") +
  geom_text(aes(label = count), vjust = -0.3, size = 3.5, color = "black") +
  scale_fill_manual(values = color_palette) +
  scale_x_discrete(expand = expansion(mult = c(0.05, 0.05))) +  
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +  
...
ggsave("./Data analysis/Figure3_logOutcome_Types.png", height = 7, width = 12)
rm(data_summary)

## Visualisation of exposure types (original Figure 4 but now combined with Figure 3 into one Sankey plot)
#Now we want to get the counts of these new categories
data_summary <- data.frame(table(data_clean$taxa_group))
colnames(data_summary) <- c("taxa_group", "count")
color_palette <- colorRampPalette(brewer.pal(12, "Paired"))(length(unique(data_summary$taxa_group)))
p2a <- ggplot(data_summary, aes(x = reorder(taxa_group, -count), y = count, fill = taxa_group)) +
  geom_bar(stat = "identity", width = 0.7) +
...
p2b <- ggplot(data_summary, aes(x = reorder(taxa_group, -count), y = log(count), fill = taxa_group)) +
  geom_bar(stat = "identity", width = 0.7) +
  labs(x = "Exposure Group", y = "Log(Total Count)") +
  geom_text(aes(label = count), vjust = -0.3, size = 3.5, color = "black") +
  scale_fill_manual(values = color_palette) +
  scale_x_discrete(expand = expansion(mult = c(0.05, 0.05))) +  
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +
...
ggsave("./Data analysis/Figure4_logExposure_Types.png", height = 7, width = 12)
rm(data_summary)

## Sankey Plot combining exposures and outcomes (new Figure 3)
#Make count data for data_summary to then make a sankey plot
#Now we want to get the counts of these new categories
data_sankey <- data_clean %>%
  group_by(outcome_group, taxa_group) %>%
  summarise(count = n()) %>%
  arrange(desc(count))
nodes <- data.frame(name = unique(c(data_sankey$taxa_group, data_sankey$outcome_group)))
data_sankey$source <- match(data_sankey$taxa_group, nodes$name) - 1
data_sankey$target <- match(data_sankey$outcome_group, nodes$name) - 1
sankey <- sankeyNetwork(Links = data_sankey, Nodes = nodes, 
                        Source = "source", Target = "target", 
                        Value = "count", NodeID = "name",
                        sinksRight = FALSE, fontSize = 12, nodeWidth = 30)
htmlwidgets::saveWidget(sankey, "./Data analysis/sankey_plot.html", selfcontained = T)
webshot::webshot("./Data analysis/sankey_plot.html", "./Data analysis/sankey_plot.png", vwidth = 1000, vheight = 800, delay = 0.5)  
sankey_grob <- grid::rasterGrob(png::readPNG("./Data analysis/sankey_plot.png"), interpolate = TRUE)
sankey_plot <- plot_grid(
  ggdraw() + draw_grob(sankey_grob),  
  ncol = 1, rel_heights = c(0.1, 1) 
)

# Final layout with the Sankey plot on the right
plot_grid(
  plot_grid(p2a, p1a, ncol = 1, rel_heights = c(1, 1), labels = c("(A)", "(B)")),  
  sankey_plot,  
  ncol = 2, rel_widths = c(5, 6), labels = c("", "(C)")
)
ggsave("./Data analysis/Figure3_Sankey.png", height = 8, width = 20)

plot_grid(
  plot_grid(p2b, p1b, ncol = 1, rel_heights = c(1, 1), labels = c("(A)", "(B)")),  
  sankey_plot,  
  ncol = 2, rel_widths = c(5, 6), labels = c("", "(C)")
)
ggsave("./Data analysis/Figure3_Sankeylog.png", height = 8, width = 20)
rm(data_summary, data_clean)

## Quality assessment within studies (Figure 5)
qa_summary1 <- quality %>%
...
## Quality assessment across studies (Figure 4)
...
ggplot(qa_long2, aes(x = ROB_metric, y = count, fill = ROB_type)) +
  geom_bar(stat = "identity", show.legend = TRUE, color = "black", size = 0.25) +  
  labs(x = "Quality Assessment Category", y = "Number of studies",
       fill = "Quality Assessment") +
  scale_fill_brewer(palette = "YlOrRd", direction = -1, labels = c("lowROB" = "High Quality", "mediumROB" = "Medium Quality", "highROB" = "Low Quality")) +  
  coord_flip() +  
  theme_minimal(base_size = 15) +  
